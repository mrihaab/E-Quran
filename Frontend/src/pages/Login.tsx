...
export const Login = () => {
  ...
  const handleLogin = async (values: { email: string; password: string }, setSubmitting: (b: boolean) => void) => {
    try {
      ...
      // Navigate based on role from backend automatically
      ...
      <div className="w-full max-w-275 grid lg:grid-cols-2 bg-white rounded-xl overflow-hidden shadow-2xl border border-primary/10">
        ...
        <div className="p-8 lg:p-16 flex flex-col justify-center">
          ...
          <div className="mb-10 text-center lg:text-left">
            ...
            <p className="text-slate-500">Enter your email and password. We will automatically log you into your respective dashboard.</p>
          </div>
          ...
          >
            {({ isSubmitting }) => (
              <Form className="space-y-6">
                ...
                <button
                  className="w-full text-white font-bold py-4 rounded-xl transition-all shadow-xl flex items-center justify-center gap-2 group bg-primary hover:bg-primary/90 shadow-primary/20"
                  type="submit"
          ...
          <div className="mt-8 p-4 bg-slate-50 rounded-lg border border-slate-200">
            <p className="text-xs text-slate-500 font-bold mb-2 uppercase tracking-wider">Demo Accounts (password: admin123):</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <p className="text-xs text-slate-600"><span className="font-semibold text-primary">Student</span>: student@equran.com</p>
              <p className="text-xs text-slate-600"><span className="font-semibold text-green-600">Teacher</span>: ahmed.teacher@equran.com</p>
              <p className="text-xs text-slate-600"><span className="font-semibold text-blue-600">Parent</span>: parent@equran.com</p>
              <p className="text-xs text-slate-600"><span className="font-semibold text-orange-600">Admin</span>: admin@equran.com</p>
            </div>
          </div>
        </div>
      </div>
